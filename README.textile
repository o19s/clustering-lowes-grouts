...
h2. Next Steps
 ...
 So after all that, my colleage Dan suggested that I needed an example of showing the payout of going through the effort of clustering.  So I added a script to pull the clusters based on all the products and just the Item Number 185276 faceted options.  The script takes the suggested facets and dumps it back into each document record in the multi valued field <code>tags_txt</code>.  Go ahead and run <code>ruby insert_clustered_data.rb</code>
